(() => {
    const modalCSS = `
        ...
        #apiClientModal {
            ...
            color: #4a4a4a; display: flex; flex-direction: column; overflow: hidden;
        }
        #apiClientModal .modal-header {
            display: flex; justify-content: space-between; align-items: center;
            border-bottom: 1px solid #f0f0f0;
            padding: 15px 25px;
            background-color: #f9f9f9;
        }
         #apiClientModal .modal-header h3 { margin: 0; font-size: 1.15em; font-weight: 500; color: #333; }
         #apiClientModal button#closeApiClientModalBtn {
             background-color: transparent; border: none; font-size: 1.8em;
             color: #aaa; padding: 0; line-height: 1; cursor: pointer;
             font-weight: 300;
         }
         #apiClientModal button#closeApiClientModalBtn:hover { background-color: transparent; color: #777; }
        #apiClientModal .modal-body { padding: 25px; overflow-y: auto; flex-grow: 1; }
        #apiClientModal label { display: block; margin-bottom: 6px; font-weight: 500; font-size: 0.95em; color: #555; }
        #apiClientModal input[type="text"],
        #apiClientModal input[type="password"],
        #apiClientModal select {
            width: 100%; padding: 10px 12px; margin-bottom: 18px; border: 1px solid #dcdcdc;
            border-radius: 5px; box-sizing: border-box; font-size: 1em; background-color: #fff;
            color: #4a4a4a;
            appearance: none;
         ...
         #apiClientModal input[type="text"]:focus,
         #apiClientModal input[type="password"]:focus,
         #apiClientModal select:focus {
             border-color: #6366F1; outline: none; box-shadow: 0 0 0 2px rgba(99, 102, 241, 0.3);
         }
        #apiClientModal .controls-row {
             display: flex;
             align-items: center;
        ...
        #apiClientModal .control-group {
            display: flex;
            align-items: center;
            gap: 10px;
            flex-basis: 50%;
        }
         #apiClientModal .control-group label {
             margin-bottom: 0;
             white-space: nowrap;
         }
         #apiClientModal .control-group select {
            margin-bottom: 0;
            flex-grow: 1;
            padding: 8px 10px;
            min-width: 110px;
         }
        #apiClientModal .toggle-switch { 
            margin-bottom: 0; 
            user-select: none; 
            display: flex; 
            align-items: center;
        ...
        #apiClientModal .toggle-switch span { 
            font-weight: 500; 
            font-size: 0.95em; 
            color: #555; 
            margin-right: 10px; 
        ...
        #apiClientModal .toggle-switch input { height: 0; width: 0; visibility: hidden; }
        #apiClientModal .toggle-switch label {
             cursor: pointer; text-indent: -9999px; width: 40px; height: 20px; background: #ddd;
             display: block; border-radius: 100px; position: relative; flex-shrink: 0;
             margin-left: auto;
        }
        #apiClientModal .toggle-switch label:after {
             content: ''; position: absolute; top: 2px; left: 2px; width: 16px; height: 16px;
             background: #fff; border-radius: 90px; transition: 0.3s;
        }
        #apiClientModal .toggle-switch input:checked + label { background: #6366F1; }
        #apiClientModal .toggle-switch input:checked + label:after { left: calc(100% - 2px); transform: translateX(-100%); }
        #apiClientModal .toggle-switch label:active:after { width: 22px; }
        #apiClientModal button#submitApiRequest {
            padding: 10px 20px; background-color: #6366F1; color: white; border: none;
            border-radius: 5px; cursor: pointer; font-size: 1em; font-weight: 500;
            transition: background-color 0.2s ease; margin-right: 5px;
        }
        #apiClientModal button#submitApiRequest:hover { background-color: #6366F1; }
        #apiClientModal #modalLog {
            margin-top: 20px; padding: 12px; border: 1px solid #eee; border-radius: 5px;
            height: 120px; overflow-y: scroll; font-size: 0.85em; line-height: 1.5; color: #666;
            background-color: #fdfdfd;
            white-space: pre-wrap;
        }
         #apiClientModal #modalLog div { margin-bottom: 3px; }
         #apiClientModal #modalLog .log-error { color: #e57373; font-weight: 500; }
        #apiClientModal #resultsTableContainer { margin-top: 20px; }
        #apiClientModal #resultsTableContainer h4 { margin-bottom: 10px; font-size: 1.05em; font-weight: 500; color: #444; }
        #apiClientModal #resultsTableContainer table {
            width: 100%; border-collapse: collapse; border: 1px solid #e8e8e8; border-radius: 5px; overflow: hidden;
        }
        #apiClientModal #resultsTableContainer th,
        #apiClientModal #resultsTableContainer td { border-bottom: 1px solid #e8e8e8; padding: 10px 12px; text-align: left; }
         #apiClientModal #resultsTableContainer tr:last-child td { border-bottom: none; }
        #apiClientModal #resultsTableContainer th {
            background-color: #f5f7fa; font-weight: 500; font-size: 0.9em; color: #555;
            text-transform: uppercase; letter-spacing: 0.5px;
        }
         #apiClientModal #resultsTableContainer td { font-size: 0.95em; color: #555; }
        #apiClientModal .modal-footer {
            border-top: 1px solid #f0f0f0; padding: 12px 25px; font-size: 0.85em;
            text-align: center; background-color: #f9f9f9;
            color: #777;
        }
        #apiClientModal .modal-footer a { color: #6366F1; text-decoration: none; }
        #apiClientModal .modal-footer a:hover { text-decoration: underline; }
